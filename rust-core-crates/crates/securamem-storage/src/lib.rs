...
impl Database {
    ...
    /// Used for demo databases that have their own schema already set up.
    pub async fn open_without_migrations(db_path: &Path) -> Result<Self> {
        // Configure SQLite options
        let options = SqliteConnectOptions::from_str(db_path.to_str().unwrap())
            .map_err(|e| SecuraMemError::Database(e.to_string()))?
            .create_if_missing(false)  // Database must already exist
            .journal_mode(SqliteJournalMode::Wal)
            .synchronous(SqliteSynchronous::Normal);

        // Connect (no migrations)
        let pool = SqlitePool::connect_with(options).await
            .map_err(|e| SecuraMemError::Database(e.to_string()))?;

        tracing::info!("Database opened (no migrations) at {:?}", db_path);
