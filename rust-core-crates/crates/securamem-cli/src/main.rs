...
enum Commands {
    ...
    /// Launch the Executive Dashboard GUI
    Dashboard {
        /// Run in demo mode with synthetic data
        #[arg(long)]
        demo: bool,
        /// Run headless (API only, no GUI window)
        #[arg(long)]
        headless: bool,
    },
}
...
async fn main() -> securamem_core::Result<()> {
    ...
    match cli.command {
        ...
        Commands::ExportAudit { output } => {
            ...
            if !db_path.exists() {
                eprintln!("❌ Database not initialized. Run 'smrust init' first.");
                std::process::exit(1);
            }
        ...
        Commands::Dashboard { demo, headless } => {
            let db_path = PathBuf::from(".securamemrust/memory.db");

            // Check if database exists (unless demo mode with separate DB)
            if !demo && !db_path.exists() {
                eprintln!("❌ Database not initialized. Run 'smrust init' first.");
                eprintln!("   Or use --demo for a demo with synthetic data.");
                std::process::exit(1);
            }

            if headless {
                tracing::info!("Dashboard headless mode not yet implemented.");
                tracing::info!("Use 'smrust serve' for the API server instead.");
                return Ok(());
            }

            println!("╔══════════════════════════════════════════════════════════════════════════════╗");
            println!("║              SECURAMEM EXECUTIVE DASHBOARD                                   ║");
            println!("║              AI Flight Recorder - Compliance Platform                        ║");
            println!("╠══════════════════════════════════════════════════════════════════════════════╣");
            if demo {
                println!("║  🎭 DEMO MODE - Using synthetic data                                         ║");
            }
            println!("║  📊 Launching dashboard window...                                             ║");
            println!("╚══════════════════════════════════════════════════════════════════════════════╝");

            let config = securamem_dashboard::DashboardConfig {
                port: 9430,
                demo_mode: demo,
                db_path,
            };

            if let Err(e) = securamem_dashboard::start_dashboard(config).await {
                tracing::error!("Dashboard crashed: {}", e);
                std::process::exit(1);
            }
